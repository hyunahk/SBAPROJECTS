...
class Service:
    ...
    def age_ordinal(this) -> object:
        ...
        test['Age'] = test['Age'].fillna(-0.5)
         # age를 편균으로 넣기도 애매하고, 다수결로 넣기도 너무 근거가 없다.
         # 특히 age는 생존률 판단에서 가중치(weight)가 상당하므로 디테일한 접근이 필요합니다.
         # 나이를 모르는 승객은 모르는 상태로 처리해야 값의 왜곡을 줄일수 있어서 -0.5라는 중간값으로 처리했습니다.
        bins = [-1, 0, 5, 12, 18, 24, 35, 60, np.inf] #이 파트는 범위를 뜻합니다.
        # -1이상 0미만...60이상 기타...
        # []에 있으니 이것은 변수명이겠군요..라고 판단해야함.
        ...
        age_title_mapping = {
            ...
            6: 'Adult',
            7: 'Senior'
        } # 이렇게 []에서 {}으로 처리하면 labels를 값으로 처리하게됨
    ...
    def fareBand_norminal(this) -> object: #요금이 다양하니 클러스터링을 하기위한 준비
        this.train = this.train.fillna({'FareBand': 1}) #FareBand는 없는 변수인데 추가함
    ...
    def embarked_norminal(this) -> object:
        ...
        #교과서 146 문자 blue = 0, green = 1, red = 2로 치환합니다.
    ...
    def name_norminal(this) -> object:
        ...
        for dataset in combine:
            dataset['Title'] = dataset.Name.str.extract('([A-Za-z]+)\.', expand=False) #한글은 [ㄱ-힣]
        for dataset in combine:
